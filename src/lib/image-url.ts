import { localFallbackForRemoteImage } from "@/lib/local-media";
...
export function normalizePublicImageUrl(src: string) {
  ...
  // Unsplash hotlinks often return HTTP 402; remap to local curated assets.
  const withoutUnsplash = localFallbackForRemoteImage(value);
  if (withoutUnsplash !== value) return withoutUnsplash;
