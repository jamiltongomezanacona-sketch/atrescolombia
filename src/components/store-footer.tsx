  ...
  return (
    <footer className="store-footer-shell mt-6 pb-24 text-white md:pb-0">
      <div className="catalog-container grid gap-6 py-7 sm:grid-cols-2 lg:grid-cols-[1.35fr_1fr_1fr_1fr] lg:gap-8">
        <div>
          <p className="text-xl font-medium tracking-[0.14em] text-gold-light">ATRES</p>
        ...
        <div>
          ...
          <ul className="mt-3 grid gap-2">
            <li>
              <Link href="/productos" className="text-sm font-normal text-white/85 transition hover:text-gold-light">
                Catalogo completo
              </Link>
            </li>
            {categoryLinks.map((link) => (
              <li key={link.key}>
                <Link href={link.href} className="text-sm font-normal text-white/85 transition hover:text-gold-light">
                  {link.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <Link
            href="/tiendas"
            className="mt-3 inline-flex text-sm font-medium text-white/85 underline-offset-4 transition hover:text-gold-light hover:underline"
          >
            Ver tiendas
          </Link>
        </div>
