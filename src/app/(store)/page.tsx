  ...
  return (
    <main>
      <section className="relative overflow-hidden border-b border-black/5 bg-black text-white">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(255,77,0,0.28),transparent_42%),radial-gradient(circle_at_80%_0%,rgba(255,234,97,0.18),transparent_36%)]" />
        <div className="store-container relative grid gap-8 py-12 md:grid-cols-[1.1fr_0.9fr] md:items-center md:py-16">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-brand">ATRES Colombia</p>
            <h1 className="mt-4 max-w-xl text-4xl font-black leading-[0.95] tracking-tight sm:text-5xl md:text-6xl">
              Moda para comprar hoy
            </h1>
            <p className="mt-4 max-w-md text-sm font-semibold leading-6 text-white/75 md:text-base">
      ...
      <section className="soft-section store-container py-6 md:py-8">
        <div className="relative grid grid-cols-4 gap-x-3 gap-y-6 sm:grid-cols-5 md:gap-y-8 lg:grid-cols-8">
          {categories.map((category) => {
            ...
            return (
              <Link key={category.slug} href={`/categoria/${category.slug}`} className="group text-center">
                <div
                  className={`mx-auto flex aspect-square w-full max-w-[96px] items-center justify-center overflow-hidden rounded-full p-1.5 shadow-soft ring-1 ring-white/60 transition duration-300 group-hover:-translate-y-1 group-hover:scale-105 ${theme.washClass}`}
                >
                  <div className="relative h-full w-full overflow-hidden rounded-full bg-white/70">
                    <Image
                      src={category.image}
                      alt={category.name}
                      fill
                ...
                <p className="mx-auto mt-3 min-h-10 max-w-[110px] text-sm font-semibold leading-5 text-ink">
                  {category.shortName}
                </p>
              </Link>
            );
        ...
        <div className="grid gap-3 md:grid-cols-3 md:gap-4">
          {promos.map((promo) => (
            <Link
              key={promo.title}
              href={promo.href}
              className={`${promo.tone} relative min-h-[150px] overflow-hidden rounded-lg p-5 shadow-soft ring-1 ring-black/5 transition duration-300 hover:-translate-y-1`}
            >
              <Image
                src={promo.image}
                alt=""
                ...
                className="object-cover opacity-[0.16] scale-105"
              />
