...
export function HomeTrustStrip() {
  return (
    <section className="home-trust-strip border-y border-[#f3c9cf]/50 bg-[#fff0f3]" aria-label="Confianza ATRES">
      <ul className="home-scroll-row atres-scroll catalog-container flex gap-2 overflow-x-auto py-2 sm:grid sm:grid-cols-2 sm:gap-2.5 sm:overflow-visible sm:py-2.5 lg:grid-cols-4">
        {HOME_TRUST_ITEMS.map((item) => (
          <li
            key={item.label}
            className="home-scroll-item flex min-h-9 min-w-[10.5rem] shrink-0 items-center gap-2 rounded-[var(--radius-card)] bg-white/70 px-3 py-1.5 text-ink ring-1 ring-black/[0.04] sm:min-h-10 sm:min-w-0 sm:bg-transparent sm:px-2 sm:ring-0"
          >
